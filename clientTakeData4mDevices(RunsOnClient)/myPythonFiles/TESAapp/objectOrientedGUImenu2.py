...
class mainWindow(Frame):
    ...
    def __init__(self,instrument,root):
        ...
        self.Xtick=np.arange(0,10*60,30)
    ...
    def dailyDataPlot(self):
        ...
        def getAndPlot():
            ...
            plt.tight_layout()
            plt.grid(True)
            plt.xticks(temp,xtick,rotation=70)
...
####
    ...
    def monthlyDataResult(self):
        ...
        def findEnergyUsed():
           ...
           tp=Toplevel(self)
           tp.title("Year : 2015")
           Label(tp,text="Total Energy used = "+Emonth+" W\nPeak Power in Month = "+peak+" W\n Duration energy generated = "+str(int(duration.hour)+int(duration.day)*24)+" hours "+str(duration.minute)+" minutes",bg="white",fg="blue").pack(side="top",fill=X)
